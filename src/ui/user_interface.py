...
class UI():
    def __init__(self):
        ...
        self.screen = pygame.display.set_mode(
            (self.height_and_width*self.scale, self.height_and_width*self.scale))
            ...
            if not self.game_over:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 1:
                        ...
                        self.chosen_piece = self.all_sprites.get_sprites_at(
                            pos_on_mbdown)
                    ...
                    if event.button == 1:
                        ...
                        if self.chosen_piece != []:
                            if self.chosen_piece_original_position != None:
                                ...
                                y_op = (self.downscale(
                                    self.chosen_piece_original_position[0]))
                                x_op = (self.downscale(
                                    self.chosen_piece_original_position[1]))
                                dx = x_np-x_op
                                dy = y_np-y_op
    ...
    def draw_board(self):
        ...
        for x in range(len(self.squaregrid)):
            for y in range(len(self.squaregrid)):
                ...
                self.screen.blit(self.tiles[square],
            ...
            for y in range(len(self.peli.lauta)):
                ...
                if self.peli.choose_square(y, x) == -5:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "rook", "b", self.scale))
                if self.peli.choose_square(y, x) == -3:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "knight", "b", self.scale))
                if self.peli.choose_square(y, x) == -4:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "bishop", "b", self.scale))
                if self.peli.choose_square(y, x) == -6:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "queen", "b", self.scale))
                if self.peli.choose_square(y, x) == -7:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "king", "b", self.scale))
                if self.peli.choose_square(y, x) == 5:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "rook", "w", self.scale))
                if self.peli.choose_square(y, x) == 3:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "knight", "w", self.scale))
                if self.peli.choose_square(y, x) == 4:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "bishop", "w", self.scale))
                if self.peli.choose_square(y, x) == 6:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "queen", "w", self.scale))
                if self.peli.choose_square(y, x) == 7:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "king", "w", self.scale))
                if self.peli.choose_square(y, x) == -1:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "pawn", "b", self.scale))
                if self.peli.choose_square(y, x) == 1:
                    self.all_sprites.add(
                        Piece(scaled_x, scaled_y, "pawn", "w", self.scale))
